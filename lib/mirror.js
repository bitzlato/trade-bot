...
function createApp(config, axios = Axios, fetcher = Fetcher) {
    ...
    async function cancelAllOrders() {
        console.info(' CANCELLING ALL ORDERS');

        const orders = (await app.axios.get(`/market/v1/private/${app.userId}/orders/`, {
            params: { limit: 999 }
        })).data;
        for (const o of orders.data) {
            try {
                await app.axios.delete(`/market/v1/private/${app.userId}/orders/${o.id}`);
                console.info(`  #${o.id}`);
            } catch (e) {
