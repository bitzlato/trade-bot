...
class Pair {
    constructor({ local, exchange, app }) {
        ...
        let [bidAmountMin, bidAmountMax] = app.config.bidAmount.split('-');
        let [askAmountMin, askAmountMax] = app.config.askAmount.split('-');
        if (!bidAmountMax) bidAmountMax = bidAmountMin;
        if (!askAmountMax) askAmountMax = askAmountMin;

        const [base, quote] = this.local.split('-');
        getCrossPrice(app.fetcher.tickers[exchange], base, quote);

        const USDprice = getCrossPrice(app.fetcher.tickers[exchange], 'USD', base).last.toFixed(8);
        this.bidAmountMin = USDprice * bidAmountMin;
        this.bidAmountMax = USDprice * bidAmountMax;
        this.askAmountMin = USDprice * askAmountMin;
        this.askAmountMax = USDprice * askAmountMax;
        console.info(`1 USD ~ ${USDprice} ${base}`);
        if (this.bidAmountMax === this.bidAmountMin) {
            console.info(`Bid order size ${this.bidAmountMin.toFixed(8)} ${base} ${app.config.bidAmount}`);
        } else {
            console.info(`Bid order size from ${this.bidAmountMin.toFixed(8)} to ${this.bidAmountMax.toFixed(8)} ${base}`);
        }
        if (this.askAmountMax === this.askAmountMin) {
            console.info(`Ask order size ${this.askAmountMin.toFixed(8)} ${base} ${app.config.askAmount}`);
        } else {
            console.info(`Ask order size from ${this.askAmountMin.toFixed(8)} to ${this.askAmountMax.toFixed(8)} ${base}`);
        }
    }
    ...
    getAmount(dir) {
        let rand;
        if (dir === 'bid') {
            rand = Math.random() * (this.bidAmountMax - this.bidAmountMin) + this.bidAmountMin;
        } else {
            rand = Math.random() * (this.askAmountMax - this.askAmountMin) + this.askAmountMin;
        }
    ...
    async cancelOrders() {
        ...
        const orders = (await this.axios.get(`/market/v1/private/${this.userId}/orders/`, {
            params: { pair: this.local, limit: 999 }
        })).data;
